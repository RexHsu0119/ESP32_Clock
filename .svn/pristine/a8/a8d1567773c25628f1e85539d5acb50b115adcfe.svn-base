...
esp_err_t esp_lcd_new_panel_st7735(const esp_lcd_panel_io_handle_t io, const esp_lcd_panel_dev_config_t *panel_dev_config, esp_lcd_panel_handle_t *ret_panel)
{
    ...
    // st7735->madctl_val |= LCD_CMD_MV_BIT; //Rex 20260319 add
    // st7735->madctl_val |= LCD_CMD_MX_BIT; //Rex 20260319 add

    ESP_LOGI(TAG, "[Rex]LCD panel st7735->madctl_val = 0x%x \n", st7735->madctl_val);
...
static const st7735_lcd_init_cmd_t vendor_specific_init_default[] = {
    ...
    {ST7735_INVOFF, (uint8_t[]){0x00}, 1, 0},                                                                                             //{ST7735_INVOFF, {0}, 0},                    // set non-inverted mode
...
static esp_err_t panel_st7735_init(esp_lcd_panel_t *panel)
{
    ...
    for (int i = 0; i < init_cmds_size; i++)
    {
        ...
        case LCD_CMD_MADCTL:
            ...
            // st7735->madctl_val |= 0x60; //Rex 20260319 add
            break;
        case LCD_CMD_COLMOD:
            is_cmd_overwritten = true;
